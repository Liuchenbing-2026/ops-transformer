...
## 当前分支的 V6 实验状态（2026-09-08）

本分支在下文已验证的 V3 基线上，另含 `e731ea414` 的显式 `comm_alg="replicated_input"` 实验，需配套 vLLM-Ascend `0433817f` 的 `mega_moe_replicated_input=true`。默认路径保持原行为；新模式仅支持 A2 BF16、EP2、H2048/I512/E256/top-k8，并由调用方归约本地专家贡献。

两 rank、四种路由分布的参考精度与 5 次同步、20 次异步重复检查通过。均匀路由旧/新相对 L2 差异约 0.002847、最大绝对差 0.000244140625，因此不宣称逐位无损。V3/V6/V6/V3 四轮筛选中，uniform2048 为 6.200238→6.090760 ms；V3 计 MegaMoe 本身，V6 还计入最终 AllReduce，属于范围不同的候选筛选，不能代替整模型对照。

V6 整模型通过语义检查与同版本重复稳定性，但一个 2K 随机 token 请求的生成文本与旧版开启、关闭结果均不同。精度门槛自动停止验收，清理完成，未进入 V6 整模型性能测试。新模式仍为实验版本，尚无正的端到端收益结论。下文复现包及完整报告对应已验证的 V3；复现 V3 时固定其提交和制品，不用 V6 HEAD 替代。
